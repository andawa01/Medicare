...
const AddService = ({ serviceId }) => {
  ...
  const [removeImage, setRemoveImage] = useState(false); // remove existing one
  ...
  // compute the days for selected month/year (respecting the month length and leap years)
  ...
  // user can't select previous dates - if current month and year selected, limit days to today
  useEffect(() => {
  ...
  // to fetch services when editing state
  useEffect(() => {
    let mounted = true;
  ...
  }, [serviceId, API_BASE]); //pre fetch for that particular service if present, otherwise form will be blank for new service
  ...
  function handleImageChange(e) {
    ...
    // remove existing image if new one selected
    setRemoveImage(false);
    setHasExistingImage(false);
  }
  ...
  //Convert selected date and time to a Date object for validation
  ...
  //prevent user for selecting past date/time for slots
  ...
  // validate that all fields are filled by user
  function validate() {
    const newErrors = {};
  ...
  // submit function for creating/updating service
