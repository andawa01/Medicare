...
// this function takes slot with date and time and returns a Date object
...
const AppointmentsPage = () => {
  const isAdmin = true; //as the main admin is logged in and is major Admin for response send by him
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setAppointments(items); //fetch all the details present in the DB and set it to the appointments state
      } catch (err) {
  ...
  // filter by speciality, date and search query
  ...
  // sort filtered by datetime in descending order (latest first)
  ...
  // if admin want to cancel an appointment
  async function adminCancelAppointment(id) {
    ...
    // don't allow if already cancelled or completed
          ...
          <div className={pageStyles.headerControlsSection}>
            <div className="flex flex-col md:flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
              <div className={pageStyles.searchContainer}>
                ...
                <input
                  placeholder="Search by doctor, patient, mobile or speciality"
                  value={query}
