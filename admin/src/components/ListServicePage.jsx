...
export default function ListServicePage() {
  ...
  // fetch services from backend and normalize data for frontend use
  ...
  //convert slots map to array slots of object
  ...
  // to start editing a service, fetch latest details and open the edit form
  async function startEdit(service) {
    ...
    if (service.id) {
      try {
        ...
        console.warn(
          "Failed to fetch latest service details, using cached data",
          e,
        );
      }
    }
  ...
  // validate slots is not previous days or time
  ...
  // prevent from duplicate slots to be present
  ...
  //format the slot for backend ex-DD-MM-YYYY with HH:MM AM/PM
  ...
  //to edit any service we save then for particular service
