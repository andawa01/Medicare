...
// Normalize the backend data tha is coming from the DB
...
const ServiceDashboard = ({ services: servicesProp = null }) => {
  ...
  async function fetchServices({ showLoading = true } = {}) {
    ...
    try {
      ...
      else if (body.services && typeof body.services === "object") {
        list = [body.services]; // convert object to array
      } else {
  ...
  }, []); //global helper to refresh the page and service again
        ...
        <div className={serviceDashboardStyles.header.container}>
          ...
          <div className={serviceDashboardStyles.refresh.container}>
            <div className={serviceDashboardStyles.refresh.countText}>
              {loading
                ? "Loading..."
                : `${filteredServices.length} services ${filteredServices.length !== 1 ? "" : ""}`}
            </div>
        ...
        {filteredServices.length > INITIAL_COUNT && (
          <div className={serviceDashboardStyles.showMore.container}>
            ...
            >
              {showAll
                ? "Show les"
                : `Show all (${filteredServices.length - INITIAL_COUNT})`}
            </button>
          </div>
        )}
      </div>
