...
export const getAppointmentsByPatient = async (req, res) => {
    try {
        ...
        const appointments = (await Appointment.find(filter)).toSorted({ date: 1, time: 1 }).lean();
...
export const cancelAppointment = async (req, res) => {
    try {
        ...
        await appt.save();
        res.json({ success: true, appointment: appt, message: "Appointment canceled successfully" });
    } catch (err) {
...
export const getStats = async (req, res) => {
    try {
        ...
        return res.json({
            success: true,
            stats: { total, recentLast7Days: recent, revenue }
        });
    } catch (err) {
...
export const getAppointmentsByDoctor = async (req, res) => {
    try {
        ...
        if (!doctorId) return res.status(400).json({ success: false, message: "Missing doctorId" });
        ...
        console.error("Get Appointments By Doctor error:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
export default { getAppointments, createAppointment, getAppointments, updateAppointment, cancelAppointment, getStats, getAppointmentsByPatient, getAppointmentsByDoctor, getRegisteredUserCount };