...
// HELPER FUNCTION
// this function converts time 12-hour format to number of minutes since midnight, which makes it easier to sort time slots. For example, "2:30 PM" would be converted to 14 * 60 + 30 = 870 minutes.
...
// this  function will remove duplicate slot and return the slot and return the slot filter by time
...
//this function accept the object or JSON string
...
export async function createDoctor(req, res) {
    try {
        ...
        const doc = new Doctor({
            ...
            fee: req.body.fee !== undefined ? Number(req.body.fee) : 0,
            schedule,
        ...
        delete out.password;
        return res.status(201).json({ token, doctor: out });
...
export async function getDoctorById(req, res) {
    try {
        ...
        return res.json({ doctor: normalizeDocForClient(doc) });
    } catch (err) {
...
export async function updateDoctor(req, res) {
    try {
        ...
        if (req.body.schedule) existing.schedule = parseScheduleInput(req.body.schedule);

        const updatable = ["name", "specialization", "experience", "qualifications", "location", "about", "fee", "availability", "success", "patients", "rating"];
        updatable.forEach((k) => { if (req.body[k] !== undefined) existing[k] = req.body[k]; });

        if (req.body.email && req.body.email !== existing.email) {
            const other = await Doctor.findOne({ email: req.body.email.toLowerCase() });
            if (other && other._id.toString() !== id) return res.status(409).json({ success: false, message: "Email already in use" });
            existing.email = req.body.email.toLowerCase();
        }

        if (req.body.password) existing.password = req.body.password;
...
export async function toggleDoctorAvailability(req, res) {
    try {
        ...
        if (doc.availability === 'boolean') doc.availability = !doc.availability;
...
export async function doctorLogin(req, res) {
    try {
        ...
        const out = normalizeDocForClient(doc.toObject());
        delete out.password;
        return res.json({ token, doctor: out });
