...
export const createServiceAppointment = async (req, res) => {
    try {
        ...
        if (!clerkUserId) return res.status(401).json({ success: false, message: "Authentication is required to create a service appointment." });
...
export const confirmServicePayment = async (req, res) => {
    try {
        ...
        res.json({ success: true, appointment: appt, message: "Service payment confirmed successfully" });
...
export const updateServiceAppointment = async (req, res) => {
    try {
        ...
        const updated = await ServiceAppointment.findOneAndUpdate({ _id: id }, { $set: update }, { new: true, runValidators: true });
...
export const getServiceAppointmentStats = async (req, res) => {
    try {
        const services = await Service.aggregate([
            ...
            { $project: { name: 1, price: 1, image: "$imageUrl", totalAppointments: 1, completed: 1, canceled: 1, earning: 1, createdAt: 1 } },
            { $sort: { createdAt: -1 } },
        ])
...
export const getServiceAppointmentsForPatient = async (req, res) => {
    try {
        const clerkUserId = resolveClerkUserId(req);
        ...
        console.error("getServiceAppointmentsForPatient error:", error);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
export default {
    ...
    getServiceAppointmentsForPatient
}